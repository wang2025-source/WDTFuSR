    ...
    for epoch in range(opt["train"]["epochs"]):  # keep running
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_print'] == 0:
                ...
                logs_dict = {'epoch': epoch, 'iter': current_step, 'lr': model.current_learning_rate()}
                logs_dict.update(logs)
                logs = logs_dict
                if opt["wandb"]:
                    wandb.log(logs)
            ...
            if current_step % opt['train']['checkpoint_test'] == 0:
                ...
                metrics = {'avg_psnr': avg_psnr, 'avg_ssim': avg_ssim}
                metrics.update({k: v for k, v in logs.items()})
                metrics.update({'epoch': epoch, 'iter': current_step, 'lr': model.current_learning_rate()})
